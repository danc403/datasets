...
# --- CONFIGURATION ---
# List of db_id integers to target for pronoun replacement (from 'works' table only)
TARGET_DB_IDS = [
    # Neanderthal Parallax
    32, 33, 36, 
    # Murderbot Diaries
    47, 50, 51, 52, 53, 54, 57, 
    # Bobiverse
    124, 127, 177, 
    # Ender / Shadow / Formic Cluster (Bean's extended universe)
    130, 131, 136, 137, 142, 143, 144, 158, 163, 164, 
    # The Expanse
    148, 149, 150, 151, 152, 155, 156, 157, 218, 
    # Earth's Children (Ayla/Jondalar)
    306, 309, 310, 311, 312, 313, 
    # The Last Kingdom
    373, 
    # Tarzan Series
    422, 428, 430, 435, 436, 444, 445, 456, 457, 458
]

# Mapping: "Name": { "base": [replacements], "possessive": [replacements] }
TARGETS = {
...
def neutralize_text(text):
    ...
    # Calculate word count to determine if this is a novel-length entry
...
def process_dataset():
    ...
    print(f"Executing Targeted Neutralization on {INPUT_FILE}...")
    count = 0
    processed_novels = 0
    ...
    with open(INPUT_FILE, 'r', encoding='utf-8') as f_in, \
        ...
        for line in f_in:
            try:
                ...
                # Check mapping requirements: 
                # 1. source_table must be 'works'
                # 2. db_id must be in our target list
                source = data.get("source_table")
                db_id = data.get("db_id")
                
                if source == "works" and db_id in TARGET_DB_IDS and "text" in data:
                    original_text = data["text"]
                    data["text"] = neutralize_text(original_text)
                    
                    # Track if changes were actually attempted based on length
                ...
                if count % 1000 == 0:
                    print(f"Scanned {count} lines... (Novels neutralized: {processed_novels})")
                    
            except json.JSONDecodeError:
                continue

    print(f"Success. Scanned {count} total lines. Neutralized {processed_novels} targeted novels.")
